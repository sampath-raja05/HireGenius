...
export function FeedbackModal({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-black border border-emerald-200/20 text-stone-100 sm:max-w-3xl max-h-[85vh] overflow-y-auto">
        <StarsBackgroundDemo />
              ...
              <div className="flex flex-col gap-2">
                {feedback.strengths?.map((s, i) => (
                  <Badge
                    key={i}
                    variant="outline"
                    className="justify-start border-emerald-400/20 text-emerald-400 whitespace-normal"
                  >
                    ✓ {s}
                  </Badge>
                ))}
              ...
              <div className="flex flex-col gap-2">
                {feedback.improvements?.map((imp, i) => (
                  <Badge
                    key={i}
                    variant="outline"
                    className="justify-start border-emerald-400/20 text-red-400 whitespace-normal"
                  >
                    ✓ {imp}
                  </Badge>
                ))}
