...
function CodeBlock({
  ...
  const [hasFinishedWriting, setHasFinishedWriting] = React.useState(false);
  const renderedCode = writing ? visibleCode : code;
  const isDone = !writing || hasFinishedWriting;

  React.useEffect(() => {
    if (!renderedCode.length || !isInView) return;

    const loadHighlightedCode = async () => {
      try {
        const { codeToHtml } = await import('shiki');

        const highlighted = await codeToHtml(renderedCode, {
          lang,
          themes,
          defaultColor: theme,
        });
    ...
    loadHighlightedCode();
  }, [lang, renderedCode, themes, isInView, theme]);
