...
export function AppointmentCard({ booking, mode, isPast = false }) {
  ...
  return (
    <>
      ...
      <article className="group relative bg-[#0f0f11] border border-white/10 transition-all duration-300 hover:-translate-y-0.5 rounded-2xl bg-linear-to-t from-transparent via-transparent to-emerald-300/10 p-7 flex flex-col gap-6 self-start">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-center gap-4 min-w-0">
            <Avatar className="w-14 h-14 border border-white/10 rounded-2xl shrink-0">
              ...
              />
              <AvatarFallback className="rounded-2xl bg-emerald-400/10 border border-emerald-400/20 text-emerald-400 text-lg font-medium">
                {person?.name?.[0] ?? "?"}
              </AvatarFallback>
            </Avatar>
              ...
              {mode === "interviewee" && person?.categories?.length > 0 && (
                <div className="flex flex-wrap gap-1 mt-1">
                  {person.categories.slice(0, 3).map((cat) => (
                    <span
                      key={cat}
                      className="text-[10px] px-2 py-0.5 rounded-md border border-emerald-400/20 bg-emerald-400/5 text-emerald-400 leading-tight"
                    >
                      {cat.replaceAll("_", " ")}
                    </span>
                  ))}
        ...
        {(streamCallId || recordingUrl || feedback) && (
          <div className="flex items-center gap-2 flex-wrap pt-1">
            {!isPast && streamCallId && isUpcoming && (
              <Button variant="emerald" size="sm" className="gap-2" asChild>
              ...
              (has?.({ plan: "starter" }) || has?.({ plan: "pro" })) && (
                <>
                  <Button
                    variant="outline"
                    size="sm"
                    className="gap-1.5 border-emerald-400/20 text-emerald-400 hover:bg-emerald-400/10 hover:border-emerald-400/40"
