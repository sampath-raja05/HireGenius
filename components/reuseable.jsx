...
export const EmeraldTitle = ({ children }) => (
  <span className="bg-linear-to-br from-emerald-300 via-emerald-400 to-emerald-600 bg-clip-text text-transparent">
    {children}
  </span>
);
export const Title = ({ children }) => (
  <span className="bg-linear-to-br from-yellow-300 via-yellow-400 to-yellow-600 bg-clip-text text-transparent">
    {children}
  </span>
);
...
export const SectionHeading = ({ gray, emerald }) => (
  <h2
    className={`font-serif text-[clamp(2rem,4vw,3rem)] leading-[1.1] tracking-[-0.025em]`}
  >
    <GrayTitle>{gray}</GrayTitle>
    <br />
    <EmeraldTitle>{emerald}</EmeraldTitle>
  </h2>
);

export default function PageHeader({ label, gray, emerald, description, right }) {
  return (
      ...
      <div className="max-w-6xl mx-auto flex items-center justify-between gap-4">
        <div>
          ...
          <h1 className="font-serif text-5xl tracking-tight mt-1">
            ...
            {emerald && <EmeraldTitle>{emerald}</EmeraldTitle>}
          </h1>
          {description && (
